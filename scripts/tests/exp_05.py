...
def apply_local_gap_attack(canvas: np.ndarray, target: np.ndarray, cfg: dict) -> int:
    ...
    if side == "top":
        row = int(cells[:, 0].min())
        row_cells = cells[cells[:, 0] == row]
        row_cells = row_cells[np.argsort(row_cells[:, 1])]
        start = max(0, (len(row_cells) - length) // 2)
        selected = row_cells[start : start + length]
    elif side == "bottom":
        row = int(cells[:, 0].max())
        row_cells = cells[cells[:, 0] == row]
        row_cells = row_cells[np.argsort(row_cells[:, 1])]
        ...
        col = int(cells[:, 1].min())
        col_cells = cells[cells[:, 1] == col]
        col_cells = col_cells[np.argsort(col_cells[:, 0])]
        start = max(0, (len(col_cells) - length) // 2)
        selected = col_cells[start : start + length]
    elif side == "right":
        col = int(cells[:, 1].max())
        col_cells = cells[cells[:, 1] == col]
        col_cells = col_cells[np.argsort(col_cells[:, 0])]
...
def expansion_count(before: np.ndarray, after: np.ndarray, target: np.ndarray) -> int:
    ...
    for x, y in np.argwhere(outside_new):
        if (
            get_cell(before, int(x) - 1, int(y)) == 1
            or get_cell(before, int(x) + 1, int(y)) == 1
            or get_cell(before, int(x), int(y) - 1) == 1
            or get_cell(before, int(x), int(y) + 1) == 1
        ):
            count += 1
...
def evaluate_repair_gate(
    ...
    if gate == "none":
        return evidence >= 0.5, "none"

    if gate == "dual":
        return evidence >= theta, "dual_pass" if evidence >= theta else "dual_reject"

    if gate == "budget":
        return (
            evidence >= theta,
            "budget_candidate" if evidence >= theta else "budget_reject",
        )

    if gate == "temporal":
        passed = evidence >= theta and temporal_age >= int(
            cfg["temporal_confirm_steps"]
        )
        return passed, "temporal_pass" if passed else "temporal_reject"

    if gate == "gap":
        passed = evidence >= theta and gap_score >= 0.75
        return passed, "gap_pass" if passed else "gap_reject"

    if gate == "combined":
        passed = evidence >= theta and gap_score >= 0.75
        return passed, "combined_candidate" if passed else "combined_reject"
...
def choose_action(
    ...
    gap = gap_evidence(canvas, x, y)
    suppression = float(cfg["suppression"])
    preserve_pass = support >= theta_preserve
    repair_pass, gate_label = evaluate_repair_gate(
        gate, evidence, gap, theta_repair, int(temporal[x, y]), cfg
    )

    if operator_type == "l2sgc" and memory > 0 and gate == "combined":
        repair_pass = repair_pass and evidence >= max(theta_repair, 0.90)
...
def save_snapshot(
    ...
    write_json(
        str(path),
        {
            ...
            "temporal": temporal.astype(int).tolist(),
        },
    )
...
def make_pareto_plot(summaries: list[dict], figures_dir: Path) -> None:
    ...
    ax.set_title("Experiment 05 TPR-FPR plane")
...
def run_condition(
    ...
    temporal = np.zeros((height, width), dtype=np.int32)
    ...
    gate = str(condition["gate"])
    theta_preserve = float(condition["theta_preserve"])
    ...
    for t in range(steps + 1):
        ...
        preserve_pass_total = 0
        preserve_fail_total = 0
        ...
        if t < steps:
            ...
            temporal = np.where(canvas == 1, temporal + 1, np.maximum(temporal - 1, 0))
            ...
            for i, pos in enumerate(operators):
                ...
                gap_values.append(float(result["gap"]))

                if int(canvas[x, y]) == 1:
                    if bool(result["preserve_pass"]):
                        preserve_pass_total += 1
                    else:
                        preserve_fail_total += 1
                else:
                    repair_candidate_total += 1
            ...
            if gate in {"budget", "combined"}:
                ...
                for cell, metas in list(proposal_meta.items()):
                    if cell not in allowed:
                        filtered_meta = [m for m in metas if int(m["delta"]) <= 0]
                        if len(filtered_meta) != len(metas):
                            repair_reject_total += len(metas) - len(filtered_meta)
                            repair_pass_total -= len(metas) - len(filtered_meta)
                        proposal_meta[cell] = filtered_meta
                        proposals[cell] = [int(m["delta"]) for m in filtered_meta]
        ...
        rows.append(
            [
                ...
                int(preserve_pass_total),
                int(preserve_fail_total),
                int(repair_candidate_total),
                ...
                float(sum(gap_values) / len(gap_values)) if gap_values else 0.0,
...
def main() -> int:
    root = repo_root()
    config_path = root / "config" / "tests" / "exp_05.yaml"
    ...
    if bool(cfg["output"]["make_plot"]):
        make_line_plot(
            ...
            "Experiment 05 balanced integrity",
        )
        make_line_plot(
            all_runs,
            figures_dir,
            ...
            "Experiment 05 target true positive rate",
        )
        make_line_plot(
            all_runs,
            figures_dir,
            ...
            "Experiment 05 outside false positive rate",
        )
        make_line_plot(
            all_runs,
            figures_dir,
            ...
            "Experiment 05 repair pass total",
        )
        make_line_plot(
            all_runs,
            figures_dir,
            "repair_reject_total",
            "repair_reject_total.png",
            "Experiment 05 repair reject total",
        )
        make_line_plot(
            all_runs,
            figures_dir,
    ...
    run_summary = {
        ...
        "figures": [
            ...
            "figures/repair_reject_total.png",
