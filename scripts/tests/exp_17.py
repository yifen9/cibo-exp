...
def load_exp11(root: Path):
    return load_module(
        root / "scripts" / "tests" / "exp_11.py", "cibo_exp_11_runtime_for_exp_17"
    )


def load_exp15(root: Path):
    return load_module(
        root / "scripts" / "tests" / "exp_15.py", "cibo_exp_15_runtime_for_exp_17"
    )
...
def apply_direct_representation(g: dict, representation: str) -> dict:
    out = dict(g)
    ...
    raise ValueError(f"unknown direct representation: {representation}")


def random_direct_genome(representation: str, rng: np.random.Generator) -> dict:
    g = {
    ...
    return apply_direct_representation(g, representation)


def mutate_direct_genome(
    ...
    return apply_direct_representation(out, representation)


def crossover_direct(
    a: dict, b: dict, representation: str, rng: np.random.Generator
) -> dict:
    g = {key: a[key] if rng.random() < 0.5 else b[key] for key in direct_keys()}
    return apply_direct_representation(g, representation)


def random_comp_genome(rng: np.random.Generator) -> dict:
    return {
        "a_dilate": float(rng.uniform(0.0, 1.0)),
        "a_bridge": float(rng.uniform(0.0, 1.0)),
        "a_halo": float(rng.uniform(0.0, 1.0)),
        "a_continuity": float(rng.uniform(0.0, 1.0)),
        "a_branch": float(rng.uniform(0.0, 1.0)),
        "a_redundancy": float(rng.uniform(0.0, 1.0)),
        "a_decay": float(rng.uniform(0.50, 0.98)),
        "theta_local": float(rng.uniform(0.15, 0.55)),
        "halo_radius": int(rng.integers(1, 3)),
        "redundancy_radius": int(rng.integers(1, 3)),
        "trace_refresh": float(rng.uniform(0.25, 1.0)),
    }


def mutate_comp_genome(g: dict, rate: float, rng: np.random.Generator) -> dict:
    out = dict(g)
    ...
    for key in comp_keys():
        ...
        if key in {"halo_radius", "redundancy_radius"}:
            out[key] = int(np.clip(int(out[key]) + int(rng.choice([-1, 1])), 1, 3))
        elif key == "a_decay":
            out[key] = float(
                np.clip(float(out[key]) + rng.normal(0.0, 0.08), 0.50, 0.98)
            )
        elif key == "theta_local":
            out[key] = float(
                np.clip(float(out[key]) + rng.normal(0.0, 0.06), 0.10, 0.70)
            )
        else:
            out[key] = float(np.clip(float(out[key]) + rng.normal(0.0, 0.15), 0.0, 1.0))
...
def crossover_comp(a: dict, b: dict, rng: np.random.Generator) -> dict:
    return {key: a[key] if rng.random() < 0.5 else b[key] for key in comp_keys()}


def random_genome(
    genome_kind: str, representation: str, rng: np.random.Generator
) -> dict:
    if genome_kind == "direct":
        return random_direct_genome(representation, rng)

    if genome_kind == "comp":
        return random_comp_genome(rng)

    raise ValueError(f"unknown genome kind: {genome_kind}")
...
def mutate_genome(
    ...
    if genome_kind == "direct":
        return mutate_direct_genome(g, representation, rate, rng)

    if genome_kind == "comp":
        return mutate_comp_genome(g, rate, rng)

    raise ValueError(f"unknown genome kind: {genome_kind}")


def crossover_genome(
    a: dict, b: dict, genome_kind: str, representation: str, rng: np.random.Generator
) -> dict:
    if genome_kind == "direct":
        return crossover_direct(a, b, representation, rng)

    if genome_kind == "comp":
        return crossover_comp(a, b, rng)
...
def continuity_evidence(canvas: np.ndarray, x: int, y: int) -> float:
...
def branch_penalty(canvas: np.ndarray, x: int, y: int) -> float:
    neighbors = (
...
def redundancy_evidence(canvas: np.ndarray, x: int, y: int, radius: int) -> float:
    x0 = max(0, x - radius)
    x1 = min(canvas.shape[0], x + radius + 1)
    y0 = max(0, y - radius)
    y1 = min(canvas.shape[1], y + radius + 1)
    w = canvas[x0:x1, y0:y1]
    local_open = int(w.sum())

    if local_open >= 5:
        return 0.75

    if local_open >= 3 and bridge_evidence(canvas, x, y) > 0:
        return 1.0

    return 0.0


def halo_evidence(trace: np.ndarray, x: int, y: int, radius: int) -> float:
    x0 = max(0, x - radius)
    x1 = min(trace.shape[0], x + radius + 1)
    y0 = max(0, y - radius)
    y1 = min(trace.shape[1], y + radius + 1)
    w = trace[x0:x1, y0:y1]
    positive = float(np.maximum(w, 0.0).sum())
    negative = float(np.maximum(-w, 0.0).sum())

    if positive <= 0:
        return 0.0

    return float(np.clip(positive / max(1.0, positive + negative), 0.0, 1.0))


def comp_support(
    canvas: np.ndarray, trace: np.ndarray, genome: dict, x: int, y: int, cfg: dict
) -> float:
    ...
    dilate = 1.0 if density >= float(genome["theta_local"]) else density
    bridge = bridge_evidence(canvas, x, y)
    halo = halo_evidence(trace, x, y, int(genome["halo_radius"]))
    continuity = continuity_evidence(canvas, x, y)
    redundancy = redundancy_evidence(canvas, x, y, int(genome["redundancy_radius"]))
    branch = branch_penalty(canvas, x, y)

    canvas[x, y] = old

    score = (
        float(genome["a_dilate"]) * dilate
        + float(genome["a_bridge"]) * bridge
        + float(genome["a_halo"]) * halo
        + float(genome["a_continuity"]) * continuity
        + float(genome["a_redundancy"]) * redundancy
        - float(genome["a_branch"]) * branch
    )

    norm = (
        abs(float(genome["a_dilate"]))
        + abs(float(genome["a_bridge"]))
        + abs(float(genome["a_halo"]))
        + abs(float(genome["a_continuity"]))
        + abs(float(genome["a_redundancy"]))
        + abs(float(genome["a_branch"]))
    )

    return float(np.clip(score / max(1.0, norm), 0.0, 1.0))


def comp_complexity(genome: dict) -> float:
    active = sum(
        1
        for key in [
            "a_dilate",
        ...
        if abs(float(genome[key])) > 0.05
    )
    weight_sum = sum(
        abs(float(genome[key]))
        for key in [
            "a_dilate",
            "a_bridge",
            "a_halo",
    ...
    return float(active + 0.25 * weight_sum)


def direct_complexity(genome: dict) -> float:
    return float(
...
def genome_complexity(genome: dict, genome_kind: str) -> float:
    if genome_kind == "direct":
        return direct_complexity(genome)

    if genome_kind == "comp":
        return comp_complexity(genome)
...
def update_trace(trace: np.ndarray, proposals: dict, cfg: dict, genome: dict) -> None:
    decay = (
        float(genome["a_decay"])
        if "a_decay" in genome
        else float(genome.get("p_decay", 0.80))
    )
    refresh = (
        float(genome["trace_refresh"])
        if "trace_refresh" in genome
        else float(genome.get("p_refresh", 0.70))
    )
    trace *= float(cfg["trace"]["decay"]) * max(0.10, decay)

    for (x, y), values in proposals.items():
        score = sum(values)
        if score > 0:
            trace[x, y] = float(cfg["trace"]["duration"]) * max(0.0, refresh)
        elif score < 0:
            trace[x, y] = -float(cfg["trace"]["duration"]) * max(0.0, refresh)
...
def run_comp_trial(
    ...
    for t in range(steps + 1):
        ...
        if t < steps:
            ...
            for pos in operators:
                x = int(pos[0])
                y = int(pos[1])
                support = comp_support(canvas, trace, genome, x, y, cfg)
    ...
    return {
        ...
        "scaffold_complexity": comp_complexity(genome),
    }
...
def run_direct_trial(
    ...
    trial["path_type"] = str(c["path_type"])
    trial["scaffold_complexity"] = direct_complexity(genome)
    return trial


def run_trial_by_kind(
    exp11, exp15, cfg: dict, condition: dict, genome: dict, case: dict, genome_kind: str
) -> dict:
    if genome_kind == "direct":
        return run_direct_trial(
            exp11,
            exp15,
            cfg,
            condition,
...
def evaluate_genome_with_modules(
    ...
    trials = [
        run_trial_by_kind(exp11, exp15, cfg, condition, genome, case, genome_kind)
        for case in cases
    ]
...
def evaluate_worker(args: tuple) -> dict:
    cfg, condition, genome, cases, genome_kind = args
    return evaluate_genome_with_modules(
        _WORKER_EXP11, _WORKER_EXP15, cfg, condition, genome, cases, genome_kind
    )
...
def evaluate_population(
    ...
    jobs = [(cfg, condition, genome, cases, genome_kind) for genome in population]
...
def evaluate_single(
    ...
    return evaluate_population(executor, cfg, condition, [genome], cases, genome_kind)[
        0
    ]
...
def matched_random_search(
    ...
    while evaluated < budget:
        ...
        candidates = [random_genome(genome_kind, representation, rng) for _ in range(n)]
        evaluations = evaluate_population(
            executor, cfg, condition, candidates, train_cases, genome_kind
        )
    ...
    test_eval = evaluate_single(
        executor, cfg, condition, best_genome, test_cases, genome_kind
    )
...
def no_scaffold_genome_comp() -> dict:
    return {
        "a_dilate": 0.0,
        "a_bridge": 0.0,
        "a_halo": 0.0,
...
def primitive_usage_count(genome: dict, genome_kind: str) -> int | None:
    if genome_kind != "comp":
        return None
...
def primitive_diversity(genome: dict, genome_kind: str) -> float | None:
    if genome_kind != "comp":
        return None
    ...
    return float(entropy / np.log(6))


def width_like_emergence_score(genome: dict, genome_kind: str) -> float | None:
    if genome_kind != "comp":
        return None

    return float(
        np.clip(
            0.50 * float(genome["a_dilate"])
            + 0.30 * float(genome["a_continuity"])
            + 0.20 * float(genome["a_halo"]),
            0.0,
            1.0,
        )
    )
...
def bridge_like_emergence_score(genome: dict, genome_kind: str) -> float | None:
    if genome_kind != "comp":
        return None

    return float(
        np.clip(
            0.45 * float(genome["a_bridge"])
            + 0.35 * float(genome["a_redundancy"])
            + 0.20 * float(genome["a_continuity"]),
            0.0,
            1.0,
        )
    )
...
def prior_leakage_score(genome: dict, genome_kind: str) -> float:
    if genome_kind == "direct":
        return 1.0

    width = width_like_emergence_score(genome, genome_kind) or 0.0
    bridge = bridge_like_emergence_score(genome, genome_kind) or 0.0
    directness = max(float(genome["a_dilate"]), float(genome["a_bridge"]))
    return float(np.clip(0.35 * width + 0.35 * bridge + 0.30 * directness, 0.0, 1.0))


def interpretability_score(genome: dict, genome_kind: str) -> float:
    if genome_kind == "direct":
        return 0.35

    usage = primitive_usage_count(genome, genome_kind) or 0
    diversity = primitive_diversity(genome, genome_kind) or 0.0
    bloat_penalty = max(0.0, usage - 4) * 0.15
    return float(np.clip(0.50 + 0.40 * diversity - bloat_penalty, 0.0, 1.0))
...
def failure_mode(summary: dict, cfg: dict) -> str:
    if robust_discovery_pass(summary, cfg):
        return "robust_compositional_discovery"

    if str(summary["search_mode"]) == "comp_random":
        return "random_compositional_reference"
    ...
    if (
        ...
        return "compositional_under_expressiveness"

    if float(summary["prior_leakage_score"]) >= float(
        cfg["metrics"]["prior_leakage_threshold"]
    ):
        return "compositional_over_expressiveness"

    if float(summary["evolved_mean_complexity"]) > 5.0:
...
def condition_summary(condition: dict, record: dict, cfg: dict) -> dict:
    genome_kind = str(condition["genome_kind"])
    ...
    out = {
        ...
        "best_genome": genome,
        "primitive_usage_count": primitive_usage_count(genome, genome_kind),
        "primitive_diversity": primitive_diversity(genome, genome_kind),
        "width_like_emergence_score": width_like_emergence_score(genome, genome_kind),
        "bridge_like_emergence_score": bridge_like_emergence_score(genome, genome_kind),
        "prior_leakage_score": prior_leakage_score(genome, genome_kind),
        "interpretability_score": interpretability_score(genome, genome_kind),
    }
...
def evolve_condition(
    ...
    with ProcessPoolExecutor(
        ...
        if search_mode == "comp_random":
            logger.info(
                jline(
                    "condition",
                    str(condition["id"]),
                    "random_compositional_start",
                    budget=pop_size * generations,
                )
            )
            ...
            population = [
                random_genome(genome_kind, representation, rng) for _ in range(pop_size)
            ]

            for gen in range(generations):
                evaluations = evaluate_population(
                    executor, cfg, condition, population, train_cases, genome_kind
                )
                ...
                logger.info(
                    jline(
                        ...
                        worst_case_success=float(
                            best_eval["worst_case_strong_success"]
                        ),
                    )
                )
                ...
                while len(next_pop) < pop_size:
                    ...
                    child = crossover_genome(a, b, genome_kind, representation, rng)
                    child = mutate_genome(
                        child, genome_kind, representation, mutation_rate, rng
                    )
            ...
            best_record["test"] = evaluate_single(
                executor, cfg, condition, best_record["genome"], test_cases, genome_kind
            )
        ...
        best_record["no_scaffold_reference"] = evaluate_single(
            executor, cfg, condition, no_scaffold_genome_comp(), test_cases, "comp"
        )
        ...
        best_record["hand_reference"] = evaluate_single(
            executor, cfg, hand_condition, hand_genome, test_cases, "direct"
        )
...
def make_plots(rows: list[dict], run_dir: Path, figures_dir: Path) -> None:
    ...
    interpretability = [float(x["interpretability_score"]) for x in rows]
    ...
    ax.set_title("Experiment 17 evolved vs matched random success")
    ...
    ax = fig.add_subplot(1, 1, 1)
    ax.bar(labels, gain)
    ax.set_title("Experiment 17 matched evolution gain")
    ax.set_xlabel("condition")
    ...
    fig = plt.figure(figsize=(10, 5))
    ax = fig.add_subplot(1, 1, 1)
    ax.bar(labels, leakage)
    ax.set_title("Experiment 17 prior leakage score")
    ax.set_xlabel("condition")
    ax.set_ylabel("prior leakage score")
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(str(figures_dir / "prior_leakage_score.png"), dpi=160)
    plt.close(fig)

    fig = plt.figure(figsize=(10, 5))
    ax = fig.add_subplot(1, 1, 1)
    ax.bar(labels, interpretability)
    ax.set_title("Experiment 17 interpretability score")
    ax.set_xlabel("condition")
    ax.set_ylabel("interpretability score")
    ax.grid(True)
    fig.tight_layout()
    fig.savefig(str(figures_dir / "interpretability_score.png"), dpi=160)
    plt.close(fig)
...
def main() -> int:
    root = repo_root()
    config_path = root / "config" / "tests" / "exp_17.yaml"
    ...
    total_units = sum(
        1
        if str(c["search_mode"]) == "comp_random"
    ...
    summary_header = [
        ...
        "primitive_diversity",
        "width_like_emergence_score",
        "bridge_like_emergence_score",
        "prior_leakage_score",
    ...
    write_csv(
        str(run_dir / "compositional_representation_summary.csv"),
        [[x.get(k) for k in summary_header] for x in rows],
        header=summary_header,
    )
    write_json(str(run_dir / "compositional_representation_summary.json"), rows)
    ...
    comp_count = sum(1 for row in rows if str(row["genome_kind"]) == "comp")
    ...
    run_summary = {
        ...
        "compositional_condition_count": int(comp_count),
        ...
        * int(cfg["evolution"]["generations"]),
        ...
        "summary_path": "compositional_representation_summary.csv",
        "summary_json_path": "compositional_representation_summary.json",
        "figures": [
            "figures/evolved_vs_matched_random_success.png",
            "figures/matched_evolution_gain.png",
            "figures/prior_leakage_score.png",
            "figures/interpretability_score.png",
        ],
        "success": True,
    }
